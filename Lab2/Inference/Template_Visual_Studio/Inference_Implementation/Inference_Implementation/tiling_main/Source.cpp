...
int main() {  
	...
	for (int main_i = 0; main_i < PROFILING_ITERATIONS; main_i++) {
		...
		*/
		...
		vector<vector<float> > conv2_weights_tiled(5*5*32, vector<float> (32, 0));
		...
		vector<float> conv1_out_flattened(conv1_out.size() * conv1_out[0].size() * conv1_out[0][0].size() , 0);
		...
		conv2_weights_tiled = conv_weights_tiling("/local/jupyter/cpre482x-lab1/Inference/Template_Visual_Studio/Test_Input0/conv2_weights.bin", 5, 5, 32, 32);
		...
		conv1_out_flattened = flatten(conv1_out);

		conv2_out_tiled = ofmap_gen_conv2_tiling(conv1_out_flattened, conv2_weights_tiled, conv2_biases, 2);
