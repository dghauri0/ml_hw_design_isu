...
vector<int32_t> get_biases_int32(const char* filename, int x, bool flag);
...
// Quantization-4bit-Specific Functions (These will be applied only to CONV2 layer)
vector<vector<vector<uint8_t> > > image_import_uint4(const char* fileName);
vector<vector<vector<vector<int8_t> > > > conv_weights_int4(const char* filename, const int x, const int y, const int z, const int w);
vector<vector<vector<uint8_t> > > next_conv_uint4_input(vector<vector<vector<float> > > &input);
vector<vector<vector<int32_t> > > ofmap_gen_conv_int32_int4(const vector<vector<vector<uint8_t> > >& input_fmap, const vector<vector<vector<vector<int8_t> > > >& weights, const vector<int32_t>& bias);
...
vector<float> scale_weights_4bit(12 , 0);
vector<float> scale_input_4bit(12, 0);
vector<float> scale_biases_4bit(12, 0);
...
vector<vector<vector<float> > > dequantized_conv1_int4(60, vector<vector<float> >(60, vector<float>(32, 0))); // CONV 1
vector<vector<vector<float> > > dequantized_conv2_int4(56, vector<vector<float> >(56, vector<float>(32, 0))); // CONV 2
...
void compute_scale_biases(int layer_num, bool flag) {
	if (flag) {
		scale_biases[layer_num] = (scale_input[layer_num] * scale_weights[layer_num]);
	}
	else {
		scale_biases_4bit[layer_num] = (scale_input_4bit[layer_num] * scale_weights_4bit[layer_num]);
	}

}

float dequantize_element(int32_t &element, bool flag) {
	 if (flag) {
		return (element / scale_biases[layer_count]);
	 }
	 else { 
		return (element / scale_biases_4bit[layer_count]);
	 }
}
...
vector<vector<vector<uint8_t> > > next_conv_uint4_input(vector<vector<vector<float> > > &input) {
	...
	abs_conv = find_abs_3d(input);
	max = find_maximum_3d(abs_conv);
	scale_input_4bit[layer_count] = (15 / max);

	vector<vector<vector<uint8_t> > > output(length, vector<vector<uint8_t> >(height, vector<uint8_t>(channels, 0)));
	for (i = 0; i < length; i++) {
		for (f = 0; f < height; f++) {
			for (j = 0; j < channels; j++) {
				output[i][f][j] = round(scale_input_4bit[layer_count] * input[i][f][j]);
			}
		}
	}
	return output;	
...
int main() {  
	...
	for (int main_i = 0; main_i < PROFILING_ITERATIONS; main_i++) {
		...
		//image vectors with different quantizations
		...
		vector<vector<vector<uint8_t> > > conv1_image_uint4(64, vector<vector<uint8_t> >(64, vector<uint8_t>(3, 0)));
		
		//conv1 weights vectors with different quantizations
		...
		vector<vector<vector<vector<int8_t> > > > conv1_weights_int4(5, vector<vector<vector<int8_t> > >(5, vector<vector<int8_t> >(3, vector<int8_t>(32, 0))));

		//bias vector
		vector<float> conv1_biases(32, 0);
		vector<int32_t> conv1_biases_int(32, 0);
		vector<int32_t> conv1_biases_int4(32, 0);

		//output with different quantizations
		vector<vector<vector<float> > > conv1_out(60, vector<vector<float> >(60, vector<float>(32, 0)));
		vector<vector<vector<int32_t> > > conv1_out_uint8_int32(60, vector<vector<int32_t> >(60, vector<int32_t>(32, 0)));
		vector<vector<vector<uint8_t> > > conv1_out_uint8(60, vector<vector<uint8_t> >(60, vector<uint8_t>(32, 0)));
		vector<vector<vector<int32_t> > > conv1_out_uint4_int32(60, vector<vector<int32_t> >(60, vector<int32_t>(32, 0)));
		vector<vector<vector<uint8_t> > > conv1_out_uint4(60, vector<vector<uint8_t> >(60, vector<uint8_t>(32, 0)));	

		auto begin = std::chrono::high_resolution_clock::now(); // Start measuring time

		printf("Layer count is: %d.\n", layer_count);

		//import images into vectors
		conv1_image = image_import("Test_Input0/input.bin");
		conv1_image_uint8 = image_import_uint8("Test_Input0/input.bin");
		conv1_image_uint4 = image_import_uint4("Test_Input0/input.bin");

		//import weights into vectors
		conv1_weights = conv_weights("Test_Input0/conv1_weights.bin", 5, 5, 3, 32);
		conv1_weights_int8 = conv_weights_int8("Test_Input0/conv1_weights.bin", 5, 5, 3, 32);
		conv1_weights_int4 = conv_weights_int4("Test_Input0/conv1_weights.bin", 5, 5, 3, 32);

		compute_scale_biases(layer_count, 1); // important. do in this order.
		compute_scale_biases(layer_count, 0);
		conv1_biases_int = get_biases_int32("Test_Input0/conv1_biases.bin", 32, 1);
		conv1_biases_int4 = get_biases_int32("Test_Input0/conv1_biases.bin", 32, 0);
		...
		conv1_out_uint8_int32 = ofmap_gen_conv_int32(conv1_image_uint8, conv1_weights_int8, conv1_biases_int);
		conv1_out_uint4_int32 = ofmap_gen_conv_int32_int4(conv1_image_uint4, conv1_weights_int4, conv1_biases_int4);
		layer_count++;

		// Dequantizaing for comparison
		conv1_out_uint8 = next_conv_uint8_input(dequantized_conv1);
		conv1_out_uint4 = next_conv_uint4_input(dequantized_conv1);
		...
		//printf("conv1out (uint4): %" PRIu8 "\n", conv1_out_uint4[0][0][0]);
		...
		vector<vector<vector<float> > > test1_inputs = intermediate_compare_reshape("Test_Input0/layer_0_output.bin", 60, 60, 32);
		...
		float accu_diff_32 = 0;
		float accu_diff_8 = 0;
		float accu_diff_4 = 0;
		
		float max_diff_32bit = 0;
		float max_diff_8bit = 0;
		float max_diff_4bit = 0;
		float curr_diff_32bit = 0;
		float curr_diff_8bit = 0;
		float curr_diff_4bit = 0;
		...
		for (i = 0; i < 60; ++i) {
			for (f = 0; f < 60; ++f) {
				for (k = 0; k < 32; ++k) {
					//printf("Real: %f\n", test1_inputs[i][f][k]);
					//printf("32 bit: %f\n", conv1_out[i][f][k]);
					//printf("8 bit: %f\n", dequantized_conv1[i][f][k]);
					//printf("4 bit: %f\n", dequantized_conv1_4bit[i][f][k]);
					curr_diff_32bit = fabs(test1_inputs[i][f][k] - conv1_out[i][f][k]);
					accu_diff_32 += curr_diff_32bit;
					if (curr_diff_32bit > max_diff_32bit) {
						max_diff_32bit = curr_diff_32bit;
					}
					
					curr_diff_8bit = fabs(test1_inputs[i][f][k] - dequantized_conv1[i][f][k]);
					accu_diff_8 += curr_diff_8bit;
					if (curr_diff_8bit > max_diff_8bit) {
						max_diff_8bit = curr_diff_8bit;
					}

					curr_diff_4bit = fabs(test1_inputs[i][f][k] - dequantized_conv1_int4[i][f][k]);
					accu_diff_4 += curr_diff_4bit;
					if (curr_diff_4bit > max_diff_4bit) {
						max_diff_4bit = curr_diff_4bit;
					}
				}
			}
		}
		printf("conv1 diff for 32 bit: %f\n", max_diff_32bit);
		printf("conv1 diff for 8 bit: %f\n", max_diff_8bit);
		printf("conv1 diff for 4 bit: %f\n", max_diff_4bit);
		printf("conv1 accumulate for 32 bit: %f\n", accu_diff_32);
		printf("conv1 accumulate for 8 bit: %f\n", accu_diff_8);
		printf("conv1 accumulate for 4 bit: %f\n", accu_diff_4);
		...
		//conv2 weight vectors with different quantizations
		vector<vector<vector<vector<float> > > > conv2_weights(5, vector<vector<vector<float> > >(5, vector<vector<float> >(32, vector<float>(32, 0))));
		vector<vector<vector<vector<int8_t> > > > conv2_weights_int8(5, vector<vector<vector<int8_t> > >(5, vector<vector<int8_t> >(32, vector<int8_t>(32, 0))));
		vector<vector<vector<vector<int8_t> > > > conv2_weights_int4(5, vector<vector<vector<int8_t> > >(5, vector<vector<int8_t> >(32, vector<int8_t>(32, 0))));

		//bias vector
		vector<int32_t> conv2_biases_int(32, 0);
		vector<int32_t> conv2_biases_int4(32, 0);
		vector<float> conv2_biases(32, 0);

		//output vectors for different quantizations
		vector<vector<vector<float> > > conv2_out(56, vector<vector<float> >(56, vector<float>(32, 0)));
		vector<vector<vector<int32_t> > > conv2_out_uint8_int32(56, vector<vector<int32_t> >(56, vector<int32_t>(32, 0)));
		vector<vector<vector<uint8_t> > > conv2_out_uint8(56, vector<vector<uint8_t> >(56, vector<uint8_t>(32, 0)));
		vector<vector<vector<int32_t> > > conv2_out_uint4_int32(56, vector<vector<int32_t> >(56, vector<int32_t>(32, 0)));
		vector<vector<vector<uint8_t> > > conv2_out_uint4(56, vector<vector<uint8_t> >(56, vector<uint8_t>(32, 0)));

		begin = std::chrono::high_resolution_clock::now(); // Start measuring time

		//import weigths into vectors
		conv2_weights = conv_weights("Test_Input0/conv2_weights.bin", 5, 5, 32, 32);
		conv2_weights_int8 = conv_weights_int8("Test_Input0/conv2_weights.bin", 5, 5, 32, 32);
		conv2_weights_int4 = conv_weights_int4("Test_Input0/conv2_weights.bin", 5, 5, 32, 32);

		compute_scale_biases(layer_count, 1);
		compute_scale_biases(layer_count, 0);
		conv2_biases_int = get_biases_int32("Test_Input0/conv2_biases.bin", 32, 1);
		conv2_biases_int4 = get_biases_int32("Test_Input0/conv2_biases.bin", 32, 0);
		conv2_biases = get_biases("Test_Input0/conv2_biases.bin", 32);
		...
		conv2_out_uint8_int32 = ofmap_gen_conv_int32(conv1_out_uint8, conv2_weights_int8, conv2_biases_int);
		conv2_out_uint4_int32 = ofmap_gen_conv_int32_int4(conv2_out_uint4, conv2_weights_int4, conv2_biases_int4);

		layer_count++;

		conv2_out_uint8 = next_conv_uint8_input(dequantized_conv1);
		conv2_out_uint4 = next_conv_uint4_input(dequantized_conv1);
		...
		//printf("conv2out (uint4): %" PRIu8 "\n", conv2_out_uint4[0][0][0]);

		
		vector<vector<vector<float> > > test2_inputs = intermediate_compare_reshape("Test_Input0/layer_1_output.bin", 56, 56, 32);
		...
		for (i = 0; i < 56; ++i) {
			for (f = 0; f < 56; ++f) {
				for (k = 0; k < 32; ++k) {
					if(test2_inputs[i][f][k] > 0) {
						printf("Real: %f\n", test2_inputs[i][f][k]);
						printf("32 bit: %f\n", conv2_out[i][f][k]);
						printf("8 bit: %f\n", dequantized_conv2[i][f][k]);
						printf("4 bit: %f\n", dequantized_conv2_int4[i][f][k]);
					}
					curr_diff_32bit = fabs(test2_inputs[i][f][k] - conv2_out[i][f][k]);
					accu_diff_32 += curr_diff_32bit;
					if (curr_diff_32bit > max_diff_32bit) {
						max_diff_32bit = curr_diff_32bit;
					}
					
					curr_diff_8bit = fabs(test2_inputs[i][f][k] - dequantized_conv2[i][f][k]);
					accu_diff_8 += curr_diff_8bit;
					if (curr_diff_8bit > max_diff_8bit) {
						max_diff_8bit = curr_diff_8bit;
					}

					curr_diff_4bit = fabs(test2_inputs[i][f][k] - dequantized_conv2_int4[i][f][k]);
					accu_diff_4 += curr_diff_4bit;
					if (curr_diff_4bit > max_diff_4bit) {
						max_diff_4bit = curr_diff_4bit;
					}
		...
		printf("conv2 diff for 32 bit: %f\n", max_diff_32bit);
		printf("conv2 diff for 8 bit: %f\n", max_diff_8bit);
		printf("conv2 diff for 4 bit: %f\n", max_diff_4bit);
		printf("conv2 accumulate for 32 bit: %f\n", accu_diff_32);
		printf("conv2 accumulate for 8 bit: %f\n", accu_diff_8);
		printf("conv2 accumulate for 4 bit: %f\n", accu_diff_4);
...
vector<vector<vector<uint8_t> > > image_import_uint4(const char* fileName) {
	...
	for (i = 0; i < 64; i++) {
		for (f = 0; f < 64; f++) {
			for (j = 0; j < 3; j++) {
				reshaped_inputs[i][f][j] = round(scale_input_4bit[layer_count] * conv1_inputs[count]);
				count++;
			}
		}
	}
	...
	for (filter = 0; filter < filter_num; filter++) {											/* # of filters and # of output channels */
		for (x = 0; x <= ifmap_lenght - filter_length; x++) {									/* length of output */
			for (y = 0; y <= ifmap_height - filter_height; y++) {								/* height of output */
				...
				switch(layer_count) {
					case 0:
						dequantized_conv1[x][y][filter] = dequantize_element(output[x][y][filter], 1);
						break;
					case 1:
						dequantized_conv2[x][y][filter] = dequantize_element(output[x][y][filter], 1);
						break;
					case 3:
						dequantized_conv3[x][y][filter] = dequantize_element(output[x][y][filter], 1);
						break;
					case 4:
						dequantized_conv4[x][y][filter] = dequantize_element(output[x][y][filter], 1);
						break;
					case 6:
						dequantized_conv5[x][y][filter] = dequantize_element(output[x][y][filter], 1);
						break;
					case 7:
						dequantized_conv6[x][y][filter] = dequantize_element(output[x][y][filter], 1);
						break;
					default:
						break;
				}
		...
		for (x = 0; x <= ifmap_lenght - filter_length; x++) {									/* length of output */
			for (y = 0; y <= ifmap_height - filter_height; y++) {								/* height of output */
				for (z = 0; z < ifmap_channel; z++) {											/* input channel */
					for (a = 0; a < filter_length; a++) {										/* filter length */
						for (b = 0; b < filter_height; b++) {									/* filter height */
							uint8_t temp = input_fmap[x + a][y + b][z] & 0x0F;
							sum += temp * weights[a][b][z][filter];		/* MultSum Accumulation */
						}
					}
				}
				...
				switch(layer_count) {
					case 0:
						dequantized_conv1_int4[x][y][filter] = dequantize_element(output[x][y][filter], 0);
						break;
					case 1:
						dequantized_conv2_int4[x][y][filter] = dequantize_element(output[x][y][filter], 0);
						break;
					case 3:
						dequantized_conv3[x][y][filter] = dequantize_element(output[x][y][filter], 0);
						break;
					case 4:
						dequantized_conv4[x][y][filter] = dequantize_element(output[x][y][filter], 0);
						break;
					case 6:
						dequantized_conv5[x][y][filter] = dequantize_element(output[x][y][filter], 0);
						break;
					case 7:
						dequantized_conv6[x][y][filter] = dequantize_element(output[x][y][filter], 0);
						break;
					default:
						break;
				}
...
vector<int32_t> ofmap_gen_dense_int32(vector<uint8_t>& input_fmap, vector<vector<int8_t> >& weights, vector<int32_t>& bias, int output_size, bool last_layer) {
	...
	for (x = 0; x < output_size; x++) {
		...
		switch(layer_count) {
			case 10:
				dequantized_dense1[x] = dequantize_element(output[x], 0);
				break;
			case 11:
				dequantized_dense2[x] = dequantize_element(output[x], 0);
				break;
			default:
				break;
		}
...
vector<vector<vector<vector<int8_t> > > > conv_weights_int4(const char* filename, const int x, const int y, const int z, const int w) {
	...
	scale_weights_4bit[layer_count] = (7 / max);
	...
	for (i = 0; i < x; i++) {
		for (f = 0; f < y; f++) {
			for (j = 0; j < z; j++) {
				for (k = 0; k < w; k++) {
					reshaped_weights[i][f][j][k] = round(scale_weights_4bit[layer_count] * conv1_weights[count]);
					count++;
				}
			}
		}
...
vector<int32_t> get_biases_int32(const char* filename, int x, bool flag) {
	...
	for (i = 0; i < x; ++i) {
		if(flag) {
			...
			biases[i] = round(scale_biases_4bit[layer_count] * conv1_biases[i]);
		}
	}
